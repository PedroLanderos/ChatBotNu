...
def pedir_no_vacio(prompt: str) -> str:
    """Pide input hasta que no sea vacío (evita saltos que rompen el flujo)."""
    while True:
        s = input(prompt)
        if s is None:
            continue
...
while activo:
    ...
    if saludo_RE.search(user):
        ...
        # Pasamos directo a la pregunta de seguimiento; así mantenemos consistencia
    else:
        # Intentamos casar con alguna intención
        encontrado = False
    ...
    # BLOQUE DE SEGUIMIENTO (siempre aparece tras manejar el turno)
    ...
    if salir_RE.search(seguir):
        ...
        # Reusar el mismo ciclo pero ya con texto no vacío
        # Reprocesamos sin perder consistencia (cae al inicio del while)
        # Simulamos que el usuario escribió esto como próxima entrada:
        # En la práctica, simplemente continua el while y pedirá input de nuevo.
        # Para procesarlo inmediatamente, podríamos “inyectar” la variable, pero
        # mantener el ciclo simple evita estados raros.
        # Truco: colocamos el texto en 'user_raw' y seguimos la vuelta.
        # (Simplificamos volviendo al inicio del while)
        # Para que se procese ya, descomenta el bloque de abajo:
        # --------------------------------------------------------
        user = seguir
        encontrado = False
        for pid, patron in patrones_compilados:
        ...
        # Y volvemos a preguntar en la siguiente iteración.
